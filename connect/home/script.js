...
const my_career_p = document.querySelector('#my_career_p')
  ...
  const getUserCard = (user) => `
    <div class="card" data-uid="${user.uid}" data-name="${user.name}" data-career="${user.career}" data-desc="${user.description}" data-tag="${user.tag}">
  ...
  const setProfile = (id, name, career, desc, tag) => {
    ...
    button_wrap.innerHTML = `
      <button class="message" onclick="startChat('${id}', '${name}')">
        Lets chat <i class="fa-solid fa-arrow-right"></i>
      </button>
    `;
  }

  const showMyInfo = () => {
    showElemUp(my_profile);
  ...
  my_ac_circle.onclick = () => showMyInfo();
hide_profile.onclick = ()=>hideElem(profile_page)
